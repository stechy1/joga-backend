...
class UserManager {
    ...
    const
        AVATAR_SIZE = 140, REMEMBER_COOKIE = "auth_token";
    ...
    public function register($data) {
        if ($data['password'] != $data['password2']) throw new MyException('Hesla nesouhlasí.');
        ...
        $time = time();
        $user = array('user_nick' => $data['username'], 'user_hash' => $hash, 'user_password' => $pass, 'user_mail' => $data['email'], 'user_first_login' => $time, 'user_last_login' => $time, 'user_activation_code' => $checkCode);
        try {
...
//        Získání údajů
        ...
        if (!$fromDb) throw new MyException('Špatné jméno nebo heslo.');

//        Ověření hesla
        $salt = $fromDb['user_salt'];
        $userID = $fromDb['user_id'];
        $password = StringUtils::createHash($password, $salt);
        $fromDb = $this->database->queryOne("SELECT user_id, auth_tokens_selector
            FROM users
            LEFT JOIN auth_tokens ON auth_tokens_id = user_id
...
//        Aktualizace stavu uživatele
    ...
    public function loginFromCookie() {
        if (!empty($_SESSION['user']['id']) || empty($_COOKIE[self::REMEMBER_COOKIE])) return false;
        ...
        $token = $fromDb['token'];
        if (!hash_equals($token, hash('sha256', base64_decode($authenticator)))) return false;
    ...
    private function setRememberCookie($userID, $selector = null) {
        ...
        $data = ['auth_tokens_token' => hash('sha256', $authenticator), 'auth_tokens_user_id' => $userID, 'auth_tokens_expires' => $time];
    ...
    public function changePassword($data) {
        if (!isset($data['oldPassword']) || !isset($data['newPassword']) || !isset($data['newPassword2'])) throw new MyException("Nejsou vyplněny všechny povinné položky");

        $oldPassword = $data['oldPassword'];
        $newPassword = $data['newPassword'];
        $newPassword2 = $data['newPassword2'];

        if ($newPassword == $oldPassword) throw new MyException("Nové heslo se nesmí shodovat se starým heslem");

        if ($newPassword != $newPassword2) throw new MyException("Nové heslo se neshoduje s kontrolním");
        ...
        if (!$fromDb) throw new MyException("Heslo se nepodařilo změnit");
    ...
    public function updateData($data, $keyArray, $password) {
        ...
        foreach ($data as $key => $value) if (in_array($key, $keyArray)) $arr['user_' . $key] = $value;

        $fromDb = $this->database->update('users', $arr, 'WHERE user_id = ? AND user_password = ?', [$userID, $hash]);

        if (!$fromDb) throw new MyException("Údaje se nepodařilo změnit");
    ...
    public function changeAvatar($avatar) {
        ...
        if (!$fromDb) throw new MyException("Změna obrázku se nepovedla");
    ...
    public function logout($changeDb = true) {
        $fromDb = null;
        if ($changeDb) $fromDb = $this->database->update('users', ['user_online' => 0], 'WHERE user_id = ?', [$_SESSION['user']['id']]);
        unset($_SESSION['user']);
        unset($_COOKIE[self::REMEMBER_COOKIE]);
        setcookie(self::REMEMBER_COOKIE, '', -1, '/');
        if (!$changeDb || $fromDb) return true;
    ...
    public function delete($password) {
        ...
        $emptyUser = array("user_password" => "", "user_role" => "1", "user_online" => "0", "user_banned" => "0", "user_activated" => "0", "user_activation_code" => "", "user_name" => "", "user_age" => "", "user_avatar" => "empty-image", "user_region" => "", "user_city" => "", "user_motto" => "", "user_skill" => "",

        );

        $fromDb = $this->database->update("users", $emptyUser, "WHERE user_id = ? AND user_password = ?", [$userID, $pass]);

        if (!$fromDb) throw new MyException("Špatné heslo");
    ...
    public function isActivated() {
        if ($_SESSION['user']['user_activated']) return $_SESSION['user']['user_activated'];
    ...
    public function checkCode($code) {
        ...
        if ($fromDb) {
            if ($_SESSION['user']) $_SESSION['user']['activated'] = 1;
            return true;
        }
