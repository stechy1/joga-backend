...
class FileManager {
    ...
    const
        FOLDER_ATTACHMENT = "attachments/", FOLDER_UPLOADS = "uploads", FOLDER_CATEGORY = "category", FOLDER_IMAGE = "image", FOLDER_TMP = "tmp", FOLDER_AVATAR = "avatar", FOLDER_FORUM_IMAGE = "forumImage";
    ...
    public static function moveFiles($sourceDir, $destDir) {
        ...
        if (!is_dir($sourceDir)) throw new MyException("Zdroj není složka");
        if (!is_dir($destDir)) throw new MyException("Cíl není složka");
    ...
    public static function moveUploadedFiles($fileName, $destDir) {
        ...
        if (!$success) throw new MyException("Nepodařilo se přesunout soubor");
    }
    ...
    public function getDirectory($name) {
        if (!array_key_exists($name, $this->folders)) throw new MyException('Požadovaná složka neexistuje');
    ...
    public function getArticleContent($categoryURL, $articleURL) {
        ...
        if (!file_exists($path)) throw new MyException("Požadovaný soubor nebyl nalezen");
