...
class Container {
    ...
    public static function getContainer() {
        if (self::$created) throw new Exception("Container už existuje");
            ...
            foreach ($lines as $line) {
                if (count($parts = explode("@Inject", $line)) > 1) {
                    ...
                    if (count($parts) > 1) {
                        ...
                        if (array_key_exists($key, $this->clasess) && !isset($this->map->$key)) $this->getInstanceOf($key);
    ...
    public function getInstanceOf($className, $arguments = null) {
        ...
        if (!array_key_exists($className, $this->clasess)) {
            ...
            if (!array_key_exists($className, $this->clasess)) return null;
        }

        if (isset($this->map->$className)) return $this->map->$className->value;
        ...
        if (!isset($this->map->$className)) $this->mapValue($className, $obj);
