...
class BootPagination {
    ...
    function process() {
        ...
        if ($this->totalrecords > $this->pagesize) {
            ...
            if ($this->pagenumber > 1) {
                if ($this->showfirst && $this->paginationstyle != 2) {
                    ...
                    $tooltip = "zobrazuji " . $firstbound . " - " . $lastbound . " záznamů z " . $this->totalrecords . " celkem";
                    // First Link
                ...
                if ($this->paginationUrl == "") $this->paginationUrl = "#";
                ...
                if ($this->paginationstyle == 2) {
                    if ($this->prevCss != "") $prevPageCss = " class=\"" . $this->prevCss . "\"";
                ...
                // Normal Links
                if ($this->paginationstyle != 2) $paginationlst .= $this->generate_pagination_links($totalpages, $this->totalrecords, $this->pagenumber, $this->pagesize);
        ...
        if ($this->paginationstyle == 2) $paginationCss = "pager";
    ...
    function generate_pagination_links($totalpages, $totalrecords, $pagenumber, $pagesize) {
        ...
        $lst = new Pagination();
        if ($this->paginationstyle == 1) $arr = $lst->advance_pagination_links($totalpages, $pagenumber); else
            $arr = $lst->simple_pagination_links($totalpages, 15, $pagenumber);
        if (count($arr) > 0) {
            foreach ($arr as $item) {
                $firstbound = (($item - 1) * $pagesize) + 1;
                $lastbound = $firstbound + $pagesize - 1;
                if ($lastbound > $totalrecords) $lastbound = $totalrecords;
                $tooltip = "zobrazuji " . $firstbound . " - " . $lastbound . " záznamů z " . $totalrecords . " celkem";
                $css = "";
                if ($item == $pagenumber) $css = " class=\"active\"";
    ...
    function generate_previous_last_links($totalpages, $totalrecords, $pagenumber, $pagesize, $showlast) {
        ...
        if ($this->paginationstyle == 2) {
            if ($this->nextCss != "") $nextPageCss = " class=\"" . $this->nextCss . "\"";
        ...
        if ($showlast && $this->paginationstyle != 2) {
            ...
            $lastbound = $firstbound + $pagesize - 1;
            if ($lastbound > $totalpages) $lastbound = $totalpages;
            $tooltip = "zobrazuji " . $firstbound . " - " . $lastbound . "záznamů z " . $totalrecords . " celkem";
    ...
    function prepareUrl($pid) {
        if ($this->paginationUrl == "") $this->paginationUrl = "#";
        if ($pid > 1) return preg_replace("/\[p\]/", $pid, $this->paginationUrl); else
