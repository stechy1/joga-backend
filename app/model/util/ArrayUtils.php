...
class ArrayUtils {
    ...
    public static function removePrefix($prefix, array $input) {
        ...
        foreach ($input as $key => $value) {
            if (strpos($key, $prefix) === 0) $key = substr($key, mb_strlen($prefix));
            if (is_array($value)) $value = self::removePrefix($value, $prefix);
            $output[$key] = $value;
        }
        return $output;
    }
    ...
    public static function addPrefix($prefix, array $input) {
        ...
        foreach ($input as $key => $value) {
            $key = $prefix . $key;
            if (is_array($value)) $value = self::addPrefix($value, $prefix);
            $output[$key] = $value;
        }
        return $output;
    }
    ...
    public static function filterKeysPrefix($prefix, array $input) {
        $output = array();
        foreach ($input as $key => $value) {
            if (mb_strpos($key, $prefix) === 0) $output[$key] = $value;
        }
        return $output;
    }
    ...
    public static function camelToSnake($inputArray) {
        ...
        foreach ($inputArray as $key => $value) {
            ...
            if (is_array($value)) $value = self::camelToSnake($value);
    ...
    public static function snakeToCamel($inputArray) {
        ...
        foreach ($inputArray as $key => $value) {
            ...
            if (is_array($value)) $value = self::snakeToCamel($value);
            $outputArray[$key] = $value;
        }
        return $outputArray;
    }
    ...
    private static function xmlEncodeElement(array $input, DOMElement $parent) {
        foreach ($input as $key => $value) {
            ...
            if (is_array($value)) self::xmlEncodeElement($value, $element); else {
