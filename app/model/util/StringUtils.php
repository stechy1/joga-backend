...
class StringUtils {
    ...
    public static function shorten($text, $length) {
        if (mb_strlen($text) - 3 > $length) $text = mb_substr($text, 0, $length - 3) . '...';
        return $text;
    }
    ...
    private static function convertToCamel($text, $separator, $uncapitalize = true) {
        ...
        if ($uncapitalize) $result = self::uncapitalize($result);
        return $result;
    }
    ...
    public static function generatePassword($addSpecialChar = false) {
        ...
        if ($addSpecialChar) $password .= $specialChars[array_rand($specialChars)];
    ...
    private static function randomStr($from, $to, $length) {
        $str = '';
        if ($length > 1) $str .= self::randomStr($from, $to, --$length);
